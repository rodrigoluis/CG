 ...
 *
 * Tessellate the famous teapot into triangular patches, and sit the
...
THREE.TeapotGeometry = function ( size, segments, bottom, lid, body, fitLid, blinn ) {
    ...
    var trueSize = this.size / (maxHeight/2);
			...
			for ( sstep = 0 ; sstep <= this.segments ; sstep++ ) {
				...
				for ( tstep = 0 ; tstep <= this.segments ; tstep++ ) {
					...
					if ( vert[0] === 0 && vert[1] === 0 )
					{
						...
						normOut.set( 0, vert[2] > maxHeight/2 ? 1 : -1, 0 );
					}
					...
					vertOut = new THREE.Vector3( trueSize*vert[0], trueSize*(vert[2] - maxHeight/2), -trueSize*vert[1] );
